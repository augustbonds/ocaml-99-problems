...
let rec last_two l =
  ...
  | [ h; h' ] -> Some (h, h')
  | _ :: h' :: t -> last_two (h' :: t)
(* could just use "t" here...  *)

let rec nth n l =
  match l with
  | [] -> None
  | h :: t -> ( match n with 0 -> Some h | n -> nth (n - 1) t)
...
let len l =
  let rec aux acc l = match l with [] -> acc | _ :: t -> aux (acc + 1) t in
  aux 0 l
...
let rec concat l1 l2 =
  match (l1, l2) with [], xs -> xs | x :: xs, y -> x :: concat xs y
